  ...
  <div class="container-fluid form">
    <div class="row">
      <div class="col-lg-12 form-col">
        <a href="<?=base_url()?>/profile/perCompletedPDF/<?= $detail['year_start'];?>/<?= $detail['year_end'];?>/<?= $detail['faculty'];?>" target="_blank" class="btn btn-success btn-sm float-right std-researchV-btn mb-3">
          <i class="fa fa-download std-researchV-icon"></i> Download as PDF </a>
        <a href="<?=base_url()?>/profile/perCompletedCSV/<?= $detail['year_start'];?>/<?= $detail['year_end'];?>/<?= $detail['faculty'];?>" class="btn btn-success btn-sm float-right std-researchV-btn mb-3 mr-1">
          <i class="fa fa-download std-researchV-icon"></i> Download as CSV </a>
        <h3><?php echo "List of Completed Researches";?> </h3>

        <?php if($per_completed): ?>
        ...
        <div class="text-center mt-2 mb-3" style="position: relative;">
          <button type="button" class="btn" data-toggle="tooltip" data-placement="top" title="See more">
             ...
             <thead>
               ...
               <tbody>
                  <?php foreach($per_completed as $r): ?>
                          <tr>
