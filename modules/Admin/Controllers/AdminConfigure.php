...
class AdminConfigure extends BaseController{
	...
	public function index(){
		...
		echo view('Modules\Admin\Views\adminConfigV', $this->data);
	...
	function set_school_year(){
		...
		if($this->request->getMethod() == 'post'){
			...
			if($this->validate($rules)){
				...
				$this->data = [
				'school_year' => $this->request->getVar('school_year', FILTER_SANITIZE_STRING),

				];


				if($this->adminConfigModel->updateSY($this->data)){
	...
	function add_faculty(){
		...
		$rules =[
				...
				'f_code' => [
					'rules' => 'required|regex_match[^(SA000[0-9][0-9]|SA000[0-9]|FA000[0-9][0-9]|FA000[0-9]|AA000[0-9][0-9]|AA000[0-9])TG(200[0-9]|201[0-9]|202[0-1])$]|is_unique[faculty.f_code]',
					'errors' => [
		...
		if($this->validate($rules)){
				...
				$this->data = [
					'f_firstname' => $this->request->getVar('f_firstname', FILTER_SANITIZE_STRING),
					'f_lastname' => $this->request->getVar('f_lastname', FILTER_SANITIZE_STRING),
	...
	public function deact_faculty($id = null){
		...
		if($this->facultyModel->deactivate($id)){
			if($this->panelModel->deactivate($id)){
			...
			session()->setTempdata('errorDeact', 'Faculty deactivated successfully', 2);
			return redirect()->to(base_url()."/admin/config");
	...
	public function act_faculty($id = null){
		...
		if($this->facultyModel->activate($id)){
			...
			session()->setTempdata('errorAct', 'Faculty activated successfully', 2);
			return redirect()->to(base_url()."/admin/config");
